...
function exportarCSV(diagnosticos) {
  ...
  const headers = [
    ...
    "Cumplimiento_Planear_%","Cumplimiento_Hacer_%","Cumplimiento_Verificar_%","Cumplimiento_Actuar_%",
    "Servicios_Recomendados","Cantidad_Servicios",
    "Tipo_Riesgo_Prioritario",
    "Cotizacion_Minima_COP","Cotizacion_Referencia_COP",
    "Normas_A_Fortalecer",
    "Fecha_Diagnostico"
  ];
  ...
  const filas = diagnosticos.map(d => {
    ...
    const normas = Object.keys(normSet).join(" | ");
    const riesgoPrioritario = parsedResp[13]?.subtipo || "";
    ...
    return [
      ...
      csvEscape(fmtMes(d.fecha_limite || "")),
      csvEscape(d.plazo ? d.plazo + " meses" : ""),
      csvEscape(d.cumplimiento_total || d.cumplimiento || 0),
      csvEscape(siIdx.length),
      csvEscape(nosIdx.length),
      csvEscape(naIdx.length),
      csvEscape(PREGUNTAS.length),
      csvEscape(pctEtapa("P")),
      csvEscape(pctEtapa("H")),
      csvEscape(pctEtapa("V")),
      csvEscape(pctEtapa("A")),
      csvEscape(servicios),
      csvEscape(nosIdx.length),
      csvEscape(riesgoPrioritario),
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.5)", zIndex: 100, overflowY: "auto", padding: "1.5rem 1rem" }}>
      <div style={{ maxWidth: 760, margin: "0 auto", background: "#fff", borderRadius: 16, overflow: "hidden", boxShadow: "0 8px 40px rgba(0,0,0,.2)" }}>
        <div className="no-print" style={{ background: "#f5f7fa", padding: ".75rem 1.5rem", display: "flex", gap: 10, alignItems: "center", borderBottom: "1px solid #e0e6f0", flexWrap: "wrap" }}>
          <button onClick={() => window.print()} style={{ background: BLUE, color: "#fff", border: "none", borderRadius: 8, padding: "8px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>⬇ Imprimir / PDF</button>
          <button onClick={() => exportarCSV([d])} style={{ background: "#0F6E56", color: "#fff", border: "none", borderRadius: 8, padding: "8px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>📊 Exportar CSV</button>
          <a href={waURL} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "#25D366", color: "#fff", borderRadius: 8, padding: "8px 18px", fontSize: 13, fontWeight: 600, textDecoration: "none" }}>
          ...
          <button onClick={onClose} style={{ background: "none", border: "1px solid #d0d7e3", borderRadius: 8, padding: "8px 18px", fontSize: 13, cursor: "pointer" }}>✕ Cerrar</button>
          <span style={{ fontSize: 12, color: "#aaa", marginLeft: "auto" }}>Recibido: {fmt(d.created_at)}</span>
        </div>
        <div id="propuesta-print">
          ...
          <div style={{ padding: "1.5rem" }}>
            {nosIdx.length === 0 ? (
              <div style={{ background: "#E8F8F2", borderRadius: 8, padding: ".75rem 1rem", fontSize: 13, color: "#0F6E56", marginBottom: "1.25rem" }}>✅ El programa cumple con todos los ítems verificados.</div>
            ) : (
            ...
            {nosIdx.length > 0 && (
              <>
                <h3 style={{ fontSize: 14, fontWeight: 700, color: BLUE, margin: "1.5rem 0 .5rem", display: "flex", alignItems: "center", gap: 7 }}>🏥 Servicios recomendados ({nosIdx.length})</h3>
                {nosIdx.map(i => {
                  const q = PREGUNTAS[i];
                ...
                <h3 style={{ fontSize: 14, fontWeight: 700, color: BLUE, margin: "1.5rem 0 .65rem", display: "flex", alignItems: "center", gap: 7 }}>📅 Plan de trabajo — {plazo} meses</h3>
                ...
                <h3 style={{ fontSize: 14, fontWeight: 700, color: BLUE, margin: "1.5rem 0 .65rem", display: "flex", alignItems: "center", gap: 7 }}>💰 Cotización — Herramienta de negociación</h3>
                ...
                <div style={{ background: "#fff", border: "1px solid #e0e6f0", borderRadius: 12, padding: "1rem 1.25rem", marginBottom: ".75rem" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 120px 120px", gap: 8, fontSize: 11, color: "#999", fontWeight: 700, textTransform: "uppercase", paddingBottom: 6, borderBottom: "1px solid #eee" }}>
                    <div>Servicio</div><div style={{ textAlign: "right" }}>Mín. negoc.</div><div style={{ textAlign: "right" }}>Ref. cliente</div>
                  </div>
                  {cotizacion.items.length > 0 ? cotizacion.items.map((item, idx) => (
                    <div key={idx} style={{ display: "grid", gridTemplateColumns: "1fr 120px 120px", gap: 8, padding: ".55rem 0", borderBottom: "1px solid #f5f7fa", fontSize: 13, alignItems: "start" }}>
                      <div><div style={{ lineHeight: 1.5 }}>{item.nombre}</div>{item.nota && <div style={{ fontSize: 11, color: "#999", marginTop: 2 }}>{item.nota}</div>}</div>
                      <div style={{ textAlign: "right", fontWeight: 600, color: "#854F0B", whiteSpace: "nowrap" }}>${item.min.toLocaleString("es-CO")}</div>
                      <div style={{ textAlign: "right", fontWeight: 600, color: BLUE, whiteSpace: "nowrap" }}>${item.max.toLocaleString("es-CO")}</div>
                    </div>
                  )) : <p style={{ fontSize: 13, color: "#999", padding: ".5rem 0" }}>Sin servicios cotizados.</p>}
                </div>
                <div style={{ background: "#f9fafb", border: "1px solid #e8ecf2", borderRadius: 8, padding: ".85rem 1rem", fontSize: 12, color: "#666", lineHeight: 1.8, marginBottom: "1rem" }}>
                  ...
                  {cotizacion.notas.map((nota, i) => <div key={i} style={{ marginBottom: 3 }}>{nota}</div>)}
                </div>
                <div style={{ background: BLUE, borderRadius: 12, padding: "1.25rem 1.5rem", color: "#fff" }}>
                  ...
                  <div style={{ fontSize: 13, opacity: .85, lineHeight: 1.6, marginBottom: ".75rem" }}>Contacte al cliente para agendar la reunión de alcance y presentar la cotización formal.</div>
                  <a href={waURL} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "#25D366", color: "#fff", borderRadius: 8, padding: "10px 18px", fontSize: 13, fontWeight: 700, textDecoration: "none", marginBottom: ".75rem" }}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f0f4f9" }}>
      <div style={{ background: "#fff", borderRadius: 16, padding: "2.5rem 2rem", width: 340, boxShadow: "0 4px 24px rgba(0,0,0,.08)" }}>
        ...
        <input type="password" value={pw} onChange={e => setPw(e.target.value)} onKeyDown={e => e.key==="Enter" && check()} placeholder="Ingrese la contraseña"
        ...
        <button onClick={check} style={{ width: "100%", background: BLUE, color: "#fff", border: "none", borderRadius: 8, padding: "11px", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Ingresar →</button>
      </div>
    </div>
  );
}
...
export default function AdminPanel() {
  ...
  const promedio = total > 0 ? Math.round(data.reduce((a, d) => a + (d.cumplimiento_total || d.cumplimiento || 0), 0) / total) : 0;
  const criticos = data.filter(d => (d.cumplimiento_total || d.cumplimiento || 0) < 50).length;
  ...
  const toggleTodos = () => {
    ...
    const todos = ids.every(id => seleccionados.has(id)) && ids.length > 0;
    setSeleccionados(prev => { const next = new Set(prev); if (todos) ids.forEach(id => next.delete(id)); else ids.forEach(id => next.add(id)); return next; });
  };
  const handleExportar = () => exportarCSV(data.filter(d => seleccionados.has(d.id)));
  const todosSeleccionados = filtered.length > 0 && filtered.every(d => seleccionados.has(d.id));
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f0f4f9" }}>
      ...
      <div style={{ background: BLUE, padding: ".85rem 1.5rem", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <button onClick={() => setAuth(false)} style={{ background: "rgba(255,255,255,.15)", border: "none", color: "#fff", borderRadius: 8, padding: "6px 14px", fontSize: 13, cursor: "pointer" }}>Cerrar sesión</button>
      </div>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "1.5rem 1rem" }}>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: "1rem", flexWrap: "wrap" }}>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍 Buscar por empresa, ciudad o responsable..."
        ...
        {seleccionados.size > 0 && (
          <div style={{ background: "#0F6E56", borderRadius: 10, padding: ".75rem 1.25rem", marginBottom: "1rem", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setSeleccionados(new Set())} style={{ background: "rgba(255,255,255,.2)", color: "#fff", border: "none", borderRadius: 8, padding: "7px 14px", fontSize: 13, cursor: "pointer" }}>Cancelar</button>
              <button onClick={handleExportar} style={{ background: "#fff", color: "#0F6E56", border: "none", borderRadius: 8, padding: "7px 16px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                📊 Exportar CSV completo ({seleccionados.size})
              </button>
            </div>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e0e6f0", overflow: "hidden" }}>
            <div style={{ padding: ".65rem 1.25rem", borderBottom: "1px solid #f0f2f5", background: "#f9fafb", display: "flex", alignItems: "center", gap: 10 }}>
              <input type="checkbox" checked={todosSeleccionados} onChange={toggleTodos} style={{ width: 16, height: 16, cursor: "pointer" }} />
            ...
            {filtered.map((d, idx) => (
              <div key={d.id} style={{ padding: "1rem 1.25rem", borderBottom: idx < filtered.length-1 ? "1px solid #f0f2f5" : "none", display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap", background: seleccionados.has(d.id) ? "#f0f9f6" : "transparent" }}>
                ...
                <div style={{ flex: "0 0 90px", textAlign: "center" }}>
                  ...
                  <Badge pct={d.cumplimiento_total || d.cumplimiento || 0} />
                </div>
                ...
                <div style={{ flex: "0 0 auto", marginLeft: "auto" }}>
                  <button onClick={() => setPropuesta(d)} style={{ background: BLUE, color: "#fff", border: "none", borderRadius: 8, padding: "9px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}>
                    📄 Ver propuesta
                  </button>
                </div>
              </div>
        ...
        <p style={{ fontSize: 12, color: "#aaa", marginTop: "1rem", textAlign: "center" }}>
          Mostrando {filtered.length} de {total} diagnósticos · Marca las casillas y exporta el CSV completo para el CRM
        </p>
      </div>
    </div>
  );
